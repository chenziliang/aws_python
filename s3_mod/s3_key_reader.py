...
class S3KeyReader(object):
    ...
    def _get_key_object(self):
        ...
        self._config[asc.key_object] = bucket.get_key(
            self._config[asc.key])
        if self._config[asc.key_object] is None:
            msg = "Failed to get S3 key={}".format(self._config[asc.key])
...
if __name__ == "__main__":
    ...
    config = {
        asc.host_name: "s3.amazonaws.com",
        tac.key_id: os.environ[tac.key_id],
        tac.secret_key: os.environ[tac.secret_key],
        asc.bucket_name: "ken-s3-testing",
        # asc.bucket_name: "calvin-billing",
        asc.key: "audit.log",
    }
    logger = logging.getLogger("abc")
    sys.stdout.write("raw reader with seek\n")
    reader = create_s3_key_reader(config, logger)
    reader.seek(10240)
    with open(config[asc.key], "w") as f:
        for data in reader:
            f.write(data)
    ...
    key_names = [
        ("kms", "audit.log.kms"),
        ("single.tar.gz reader", "audit.log.single.tar.gz"),
        ("raw reader", "audit.log"),
        ("single.tar reader", "audit.log.single.tar"),
        ("multiple.tar reader", "audit.log.multiple.tar"),
        ("single.tar.gz reader", "audit.log.single.tar.gz"),
        ("multiple.tar.gz reader", "audit.log.multiple.tar.gz"),
        ("single.tar.bz2 reader", "audit.log.single.tar.bz2"),
        ("multiple.tar.bz2 reader", "audit.log.multiple.tar.bz2"),
        ("dir.tar reader", "audit.dir.tar"),
        ("dir.tar.gz reader", "audit.dir.tar.gz"),
        ("dir.tar.bz2 reader", "audit.dir.tar.bz2"),
        ("single.gz reader", "audit.log.single.gz"),
        ("single.zip reader", "audit.log.single.zip"),
        ("multiple.zip reader", "audit.log.multiple.zip"),
#        ("single.zip reader", "743229228294-aws-billing-detailed-line-items-with-resources-and-tags-2015-10.csv.zip"),
    ]

    for msg, key_name in key_names:
        sys.stdout.write("=======" + msg + "=====\n")
        config[asc.key_object] = None
        config[asc.key] = key_name
        reader = create_s3_key_reader(config, logger)
        sys.stdout.write("s: read_pos={}, key_name={}, total_size={}\n".format(
            reader.tell(), key_name, reader.size()))

        with open(key_name, "w") as f:
            for data in reader:
                f.write(data)

        sys.stdout.write("e: read_pos={}, key_name={}, total_size={}\n".format(
            reader.tell(), key_name, reader.size()))
